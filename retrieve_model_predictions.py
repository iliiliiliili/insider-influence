...
def get_model_predictions(
    ...
    path_model_parameter_summary = [
        ...
        if os.path.isdir(os.path.join(path_model, possible_folder))
    ][0]

    path_model_parameter_summary = os.path.join(
        path_model_parameter_summary, os.listdir(path_model_parameter_summary)[0]
    )
    ...
    arguments = [
        "--result-dir",
        "./results/model_reevaluation_trash",
        "--file-dir",
        data_path,
        "--model",
        hparams["model"],
        "--dropout",
        str(hparams["drop-out"]),
        "--data-split-seed",
        str(int(hparams["data-split-seed"])),
        "--seed",
        str(int(hparams["seed"])),
    ]
    ...
    if hparams["model"] == "gat":
        ...
        arguments.extend(["--hidden-units", hidden_units, "--heads", heads])
    ...
    sample_securities = [
        security_map[security_id] for security_id in security_ids[test_index]
    ]
    ...
    model.load_state_dict(
        torch.load(path_model_checkpoint)
    )  # , map_location=torch.device('cuda:1')
    model.to(device)
    model.eval()
    valid_loss, best_thr, valid_stats = evaluate(
        model, args, class_weight, data_loader["valid"]
    )
    assert hparams["best_threshold"] == best_thr, print(
        f'Best thresholds do not match {hparams["best_threshold"]} vs {best_thr}'
    )

    _, _, stats = evaluate(model, args, class_weight, test_loader, best_thr=best_thr)
    ...
    df = pd.DataFrame(
        ...
        index=["ego_id", "owner_id", "security", "prediction", "label", "score"],
    ).T
